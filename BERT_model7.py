#%%
'''PRESENT'''
print('이번 BERT 모델 7은 AIDS, BZR, COX2, DHFR에 대한 실험 파일입니다. 기존 파일과의 차이점은 마스크 토큰 재구성을 통한 프리-트레인은 같지만 인접행렬에 대한 재구성을 통한 프리트레인 학습이 이루어지며, 이후 기존과 같이 노드 특성을 재구성하는 모델을 통해 이상을 탐지합니다.')
...
def run(dataset_name, random_seed, dataset_AN, trial, device=device):
    ...
    # BERT 모델 저장 경로
    bert_save_path = f'/home1/rldnjs16/graph_anomaly_detection/BERT_model/Class/all_pretrained_bert_{dataset_name}_fold{trial}_nhead{n_head}_seed{random_seed}_BERT_epochs{BERT_epochs}_gcnall{hidden_dims[-1]}_try7.pth'
