...
#%%
def train(model, train_loader, recon_optimizer, epoch, dataset_name, device):
    model.train()
    total_loss = 0
    num_sample = 0
    ...
    for data in train_loader:
        ...
        # Forward pass
        x_recon, stats_pred = model(x, edge_index, batch, num_graphs)
...
class GRAPH_AUTOENCODER(nn.Module):
    ...
    def forward(self, x, edge_index, batch, num_graphs):
        ...
        # Graph embeddings via mean pooling
        graph_embeddings = []
        start_idx = 0
        for i in range(num_graphs):
            mask = (batch == i)
