...
def train(model, train_loader, recon_optimizer, device, epoch, dataset_name, cluster_centers=None):
    ...
    total_loss = 0
    num_sample = 0
    reconstruction_errors = {'mean': [], 'max': []}
    ...
    for data in train_loader:
        ...
        # Forward pass
        graph_representations, x_recon, stats_pred, adj_recon_list = model(
            x, edge_index, batch, num_graphs, is_pretrain=False
        )
        
        # 클러스터링 (각 풀링 방식별로)
        cluster_centers = {'mean': None, 'max': None}
        if epoch % 5 == 0:
            for pooling_type in ['mean', 'max']:
                graph_representations_np = graph_representations[pooling_type].detach().cpu().numpy()
                kmeans = KMeans(n_clusters=n_cluster, random_state=random_seed)
                kmeans.fit(graph_representations_np)
                cluster_centers[pooling_type] = kmeans.cluster_centers_
        ...
        for i in range(num_graphs):
            ...
            if epoch % 5 == 0:
                ...
                # 각 풀링 방식별로 클러스터링 거리 계산
                for pooling_type in ['mean', 'max']:
                    graph_vec = graph_representations[pooling_type][i].detach().cpu().numpy()
                    if cluster_centers[pooling_type] is not None:
                        distances = cdist([graph_vec], cluster_centers[pooling_type], metric='euclidean')
                        min_distance = distances.min().item() * gamma
                        
                        reconstruction_errors[pooling_type].append({
        ...
        stats_loss = 0
        for pooling_type in ['mean', 'max']:
            stats_loss += persistence_stats_loss(stats_pred[pooling_type], true_stats)
        stats_loss = stats_loss / 2  # 평균
    ...
    if epoch % 5 == 0:
        ...
        for pooling_type in ['mean', 'max']:
            ...
            recon_errors = [point['reconstruction'] for point in reconstruction_errors[pooling_type]]
            cluster_errors = [point['clustering'] for point in reconstruction_errors[pooling_type]]
            
            ax1.scatter(recon_errors, cluster_errors, c='blue', alpha=0.6)
            ax1.set_xlabel('Reconstruction Error')
            ax1.set_ylabel('Clustering Distance')
            ax1.set_title(f'Training Error Distribution ({pooling_type} pooling) (Epoch {epoch})')
            ax1.grid(True)

            ax2.scatter(recon_errors, cluster_errors, c='blue', alpha=0.6)
            ax2.set_xlabel('Reconstruction Error')
            ax2.set_ylabel('Clustering Distance')
            ax2.set_title(f'Training Error Distribution - Log Scale ({pooling_type} pooling) (Epoch {epoch})')
            ax2.set_xscale('log')
            ax2.set_yscale('log')
            ax2.grid(True)

            plt.tight_layout()
            save_path = f'/home1/rldnjs16/graph_anomaly_detection/error_distribution_plot/AB_CLS/plot/{dataset_name}_time_{current_time}/train_error_distribution_epoch_{epoch}_{pooling_type}.png'
...
def evaluate_model(model, test_loader, cluster_centers, n_clusters, gamma_clusters, random_seed, 
                  reconstruction_errors, epoch, dataset_name, device):
    model.eval()
    results = {'mean': {}, 'max': {}}
    ...
    with torch.no_grad():
        for pooling_type in ['mean', 'max']:
            ...
            for data in test_loader:
                ...
                # Forward pass
                graph_representations_dict, x_recon, stats_pred_dict, adj_recon_list = model(
                    x, edge_index, batch, num_graphs, is_pretrain=False
                )
                
                # 현재 풀링 타입의 그래프 표현 선택
                graph_representations = graph_representations_dict[pooling_type]
                graph_representations_np = graph_representations.detach().cpu().numpy()
                ...
                for i in range(num_graphs):
                    ...
                    # 클러스터링 거리 계산 (현재 풀링 타입의 클러스터 센터 사용)
                    graph_vec = graph_representations_np[i].reshape(1, -1)
                    distances = cdist(graph_vec, cluster_centers[pooling_type], metric='euclidean')
                    min_distance = distances.min().item() * gamma
            ...
            train_data = np.array([[error['reconstruction'], error['clustering']] 
                                 for error in reconstruction_errors[pooling_type] if error['type'] == 'train_normal'])
            ...
            print(f'{pooling_type} bandwidth: {bandwidth}')
            ...
            results[pooling_type] = {
                ...
                'recall': recall,
                'f1': f1,
                'total_loss_normal': total_loss_ / sum(all_labels == 0),
                'total_loss_anomaly': total_loss_anomaly_ / sum(all_labels == 1),
                'scores': all_scores,
                'labels': all_labels,
                'reconstruction_errors': reconstruction_errors_test,
                'visualization_data': {
                    'normal': [{'reconstruction': e['reconstruction'], 'clustering': e['clustering']}
                             for e in reconstruction_errors_test if e['type'] == 'test_normal'],
                    'anomaly': [{'reconstruction': e['reconstruction'], 'clustering': e['clustering']}
                               for e in reconstruction_errors_test if e['type'] == 'test_anomaly']
                }
            }
            ...
            ax1.set_xlabel('Reconstruction Error')
            ax1.set_ylabel('Clustering Distance')
            ax1.set_title(f'Density-based Anomaly Detection ({pooling_type} pooling)')
            ...
            ax2.set_title(f'Distribution of Anomaly Scores ({pooling_type} pooling)')
            ax2.legend()
            
            plt.tight_layout()
            save_path = f'/home1/rldnjs16/graph_anomaly_detection/density_analysis/{dataset_name}_time_{current_time}/epoch_{epoch}_fold_{trial}_{pooling_type}.png'
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            plt.savefig(save_path)
            plt.close()
...
class GRAPH_AUTOENCODER(nn.Module):
    def __init__(self, num_features, hidden_dims, max_nodes, nhead_BERT, 
                 num_layers_BERT, dropout_rate=0.1, pooling_type='mean'):
        super().__init__()
        ...
        self.stats_predictor_mean = nn.Sequential(
            nn.Linear(hidden_dims[-1], 5)
        )
        self.stats_predictor_max = nn.Sequential(
            nn.Linear(hidden_dims[-1], 5)
        )
    ...
    def forward(self, x, edge_index, batch, num_graphs, mask_indices=None, is_pretrain=False, edge_training=False):
        if is_pretrain:
            ...
            # 두 가지 풀링 방식으로 그래프 임베딩 생성
            graph_representations_mean = []
            graph_representations_max = []
            start_idx = 0
            ...
            for i in range(num_graphs):
                ...
                # Mean pooling
                pooled_mean = torch.mean(graph_nodes, dim=0)
                graph_representations_mean.append(pooled_mean)
                
                # Max pooling
                pooled_max = torch.max(graph_nodes, dim=0)[0]
                graph_representations_max.append(pooled_max)
                
                start_idx = end_idx

            # 그래프 레벨 표현
            graph_representations_mean = torch.stack(graph_representations_mean)
            graph_representations_max = torch.stack(graph_representations_max)
            
            # 노드 특징 재구성
            x_recon = self.feature_decoder(u)
            
            # 통계량 예측 (두 가지 풀링 각각)
            stats_pred_mean = self.stats_predictor_mean(graph_representations_mean)
            stats_pred_max = self.stats_predictor_max(graph_representations_max)
            ...
            for i in range(num_graphs):
                ...
                current_embeddings = u[start_idx:end_idx]
            ...
            return {
                'mean': graph_representations_mean,
                'max': graph_representations_max
            }, x_recon, {
                'mean': stats_pred_mean,
                'max': stats_pred_max
            }, adj_recon_list
...
def run(dataset_name, random_seed, dataset_AN, trial, device=device, epoch_results=None):
    if epoch_results is None:
        epoch_results = {'mean': {}, 'max': {}}
    epoch_interval = 10  # 10 에폭 단위로 비교
    
    set_seed(random_seed)    
    all_results = {'mean': [], 'max': []}
    split = splits[trial]
    ...
    for epoch in range(1, epochs+1):
        ...
        if epoch % log_interval == 0:
            eval_results = evaluate_model(
                model, test_loader, train_cluster_centers, 
                n_cluster, gamma_cluster, random_seed, 
                train_errors, epoch, dataset_name, device
            )
            
            # 각 풀링 방식별 결과 처리
            for pooling_type in ['mean', 'max']:
                results = eval_results[pooling_type]
                
                # 결과 저장
                test_loss = results['total_loss_normal']
                test_loss_anomaly = results['total_loss_anomaly']
                auroc = results['auroc']
                auprc = results['auprc']
                ...
                all_results[pooling_type].append(
                    (auroc, auprc, precision, recall, f1, test_loss, test_loss_anomaly)
                )
                
                # wandb 로깅
                wandb.log({
                    f"{pooling_type}/epoch": epoch,
                    f"{pooling_type}/train_loss": train_loss,
                    f"{pooling_type}/val_loss": test_loss,
                    f"{pooling_type}/val_loss_anomaly": test_loss_anomaly,
                    f"{pooling_type}/auroc": auroc,
                    f"{pooling_type}/auprc": auprc,
                    f"{pooling_type}/precision": precision,
                    f"{pooling_type}/recall": recall,
                    f"{pooling_type}/f1": f1
                })
                
                # 결과 출력
                print(f'\n{pooling_type.upper()} Pooling Results:')
                print(f'AUROC = {auroc:.4f}, AUPRC = {auprc:.4f}, F1 = {f1:.4f}')
                
                # 10 에폭마다 결과 저장
                if epoch % epoch_interval == 0:
                    if epoch not in epoch_results[pooling_type]:
                        epoch_results[pooling_type][epoch] = {
                            'aurocs': [], 'auprcs': [], 'precisions': [], 
                            'recalls': [], 'f1s': []
                        }
                    
                    epoch_results[pooling_type][epoch]['aurocs'].append(auroc)
                    epoch_results[pooling_type][epoch]['auprcs'].append(auprc)
                    epoch_results[pooling_type][epoch]['precisions'].append(precision)
                    epoch_results[pooling_type][epoch]['recalls'].append(recall)
                    epoch_results[pooling_type][epoch]['f1s'].append(f1)
                
                # 최고 성능 업데이트
                if auroc > best_auroc[pooling_type]:
                    best_auroc[pooling_type] = auroc
...
if __name__ == '__main__':
    ...
    for trial in range(n_cross_val):
        ...
        for pooling_type in ['mean', 'max']:
            ad_aucs[pooling_type].append(best_auroc[pooling_type])
    ...
    for pooling_type in ['mean', 'max']:
        ...
        for epoch in epoch_results[pooling_type].keys():
            epoch_means[epoch] = {
                'auroc': np.mean(epoch_results[pooling_type][epoch]['aurocs']),
                'auprc': np.mean(epoch_results[pooling_type][epoch]['auprcs']),
                'precision': np.mean(epoch_results[pooling_type][epoch]['precisions']),
                'recall': np.mean(epoch_results[pooling_type][epoch]['recalls']),
                'f1': np.mean(epoch_results[pooling_type][epoch]['f1s'])
            }
            epoch_stds[epoch] = {
                'auroc': np.std(epoch_results[pooling_type][epoch]['aurocs']),
                'auprc': np.std(epoch_results[pooling_type][epoch]['auprcs']),
                'precision': np.std(epoch_results[pooling_type][epoch]['precisions']),
                'recall': np.std(epoch_results[pooling_type][epoch]['recalls']),
                'f1': np.std(epoch_results[pooling_type][epoch]['f1s'])
            }
        
        # 최고 성능 에폭 찾기
        best_epoch = max(epoch_means.keys(), key=lambda x: epoch_means[x]['auroc'])
        
        print(f"\n{pooling_type.upper()} Pooling Performance at every 10 epochs:")
        for epoch in sorted(epoch_means.keys()):
            print(f"Epoch {epoch}: AUROC = {epoch_means[epoch]['auroc']:.4f} ± {epoch_stds[epoch]['auroc']:.4f}")
        
        print(f"\nBest average performance ({pooling_type} pooling) at epoch {best_epoch}:")
        ...
        final_auroc_mean = np.mean(ad_aucs[pooling_type])
        final_auroc_std = np.std(ad_aucs[pooling_type])
        print(f'Final {pooling_type.upper()} Pooling Results: AUC: {final_auroc_mean*100:.2f}±{final_auroc_std*100:.2f}')
    ...
    with open(results_path, 'w') as f:
        json.dump({
            'mean_pooling': {
                ...
                'final_auroc_mean': float(np.mean(ad_aucs['mean'])),
                'final_auroc_std': float(np.std(ad_aucs['mean']))
            },
            'max_pooling': {
                'epoch_means': epoch_means,
                'epoch_stds': epoch_stds,
                'best_epoch': int(best_epoch),
                'final_auroc_mean': float(np.mean(ad_aucs['max'])),
                'final_auroc_std': float(np.std(ad_aucs['max']))
            }
        }, f, indent=4)
