#%%
'''PRESENT'''
print('이번 BERT 모델 8은 AIDS, BZR, COX2, DHFR에 대한 실험 파일입니다. 기존 7 파일과의 차이점은 마스크 토큰 재구성을 통한 프리-트레인은 같지만, 이후 태스크에서 노드 재구성, 엣지 재구성을 통해 그래프 이상 탐지 모델을 학습하고 탐지한다는 점이 특징입니다.')
...
def run(dataset_name, random_seed, dataset_AN, trial, device=device):
    ...
    # BERT 모델 저장 경로
    bert_save_path = f'/home1/rldnjs16/graph_anomaly_detection/BERT_model/all_pretrained_bert_{dataset_name}_fold{trial}_nhead{n_head}_seed{random_seed}_BERT_epochs{BERT_epochs}_gcnall{hidden_dims[-1]}_try6_.pth'
