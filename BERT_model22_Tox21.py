...
from modules.utils import set_seed, set_device, EarlyStopping, get_ad_dataset_Tox21, adj_original, split_batch_graphs, process_batch_graphs, scott_rule_bandwidth, loocv_bandwidth_selection
...
parser.add_argument("--eval-density", action="store_false")
...
eval_density: bool = args.eval_density
...
def run(dataset_name, random_seed, device=device, epoch_results=None):
    ...
    # BERT 모델 저장 경로
    bert_save_path = f'/home1/rldnjs16/graph_anomaly_detection/BERT_model/Tox21/all_pretrained_bert_{dataset_name}_nhead{n_head_BERT}_seed{random_seed}_BERT_epochs{BERT_epochs}_try21.pth'
    ...
    for epoch in range(1, epochs+1):
        ...
        train_loss, num_sample, train_cluster_centers, train_errors = train(
            model, train_loader, recon_optimizer, device, epoch
        )
        ...
        if epoch % log_interval == 0:
            ...
            # 시각화 데이터 저장
            json_path = f'/home1/rldnjs16/graph_anomaly_detection/error_distribution_plot/json/{dataset_name}_time_{current_time}/error_distribution_epoch_{epoch}.json'
            with open(json_path, 'w') as f:
