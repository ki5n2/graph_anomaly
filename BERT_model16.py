...
def train(model, train_loader, recon_optimizer, device, epoch):
    ...
    for data in train_loader:
        ...
        for i in range(num_graphs):
            # i번째 그래프에 해당하는 노드 마스크
            graph_mask = (batch == i)
            num_nodes = graph_mask.sum().item()
            
            # i번째 그래프의 엣지 인덱스 추출
            graph_edge_index = edge_index[:, graph_mask[edge_index[0]] & graph_mask[edge_index[1]]]
            graph_edge_index = graph_edge_index - start_idx  # 노드 인덱스 조정
            
            # i번째 그래프의 노드 라벨
            graph_node_label = node_label[start_idx:start_idx + num_nodes]
        ...
        # 배치 내 모든 그래프의 통계량을 텐서로 변환
        graph_stats = torch.stack(graph_stats).to(device)  # [batch_size, 8]
        stats_loss = F.mse_loss(stats_pred, graph_stats)
