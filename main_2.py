...
from module.loss import info_nce_loss, Triplet_loss, loss_cal, loss_func
...
def train(model, train_loader, optimizer, max_nodes, device):
    ...
    for data in train_loader:
        ...
        for i in range(data.num_graphs): 
            ...
            # Structure and attribute reconstruction loss
            structure_cost, attribute_cost = loss_func(adj[i], adj_recon_list[i], data.x[start_node:end_node], x_recon[start_node:end_node])
            loss += structure_cost + attribute_cost
            
            start_node = end_node
        
        # Add new loss components
        loss += contrastive_loss + triplet_loss
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            for i in range(data.num_graphs): 
                ...
                if data[i].y.item() == 0:
                    total_loss += adj_loss / 400 + node_loss
                else:
                    total_loss_anomaly += adj_loss / 400 + node_loss
