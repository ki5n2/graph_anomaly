#%%
'''PRESENT'''
print('이번 BERT 모델 15_은 AIDS, BZR, COX2, DHFR에 대한 실험 파일입니다. 마스크 토큰 재구성을 통한 프리-트레인이 이루어집니다. 이후 기존과 같이 노드 특성을 재구성하는 모델을 통해 이상을 탐지합니다. 기존 파일과 다른 점은 성능 평가 결과 비교를 코드 내에서 수행하고자 하였으며, 해당 파일만 실행하면 결과까지 한 번에 볼 수 있도록 하였습니다. 또한, 재구성 오류에 대한 정규화가 이루어져 있습니다. 추가로 훈련 데이터에 대한 산점도와 로그 스케일이 적용되어 있습니다. 그리고 2D density estimation이 적용되어 있습니다. 그리고 분자량을 맞추는 과정이 반영되어 있습니다. 밀도 추정 이상 스코어')
...
def run(dataset_name, random_seed, dataset_AN, trial, device=device, epoch_results=None):
    ...
    for epoch in range(1, epochs+1):
        ...
        if epoch % log_interval == 0:
            ...
            auroc, auprc, precision, recall, f1, test_loss, test_loss_anomaly, all_scores, all_labels, test_errors, visualization_data, bandwidths_results, compare_bandwidths_result = evaluate_model_with_density(model, test_loader, cluster_centers, n_cluster, gamma_cluster, random_seed, epoch, trial, train_errors, device)
