...
from modules.utils import set_seed, set_device, EarlyStopping, get_ad_split_TU, get_data_loaders_TU, adj_original, get_ad_dataset_Tox21
...
parser.add_argument("--BERT-epochs", type=int, default=50)
...
def run(dataset_name, random_seed, device=device):
    ...
    # BERT 모델 저장 경로
    bert_save_path = f'/home1/rldnjs16/graph_anomaly_detection/BERT_model/Tox21/pretrained_bert_{dataset_name}_nhead{n_head}_seed{random_seed}_BERT_epochs{BERT_epochs}_gcnall{hidden_dims[-1]}_try7.pth'
    ...
    if os.path.exists(bert_save_path):
        ...
        print("Saving pretrained task1 BERT...")
